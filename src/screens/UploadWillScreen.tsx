...
import { uploadWillStyles as styles } from './UploadWillScreen.styles';
...
const UploadWillScreen: React.FC<UploadWillScreenProps> = ({ navigation, route }) => {
  ...
  const inferWillType = (will: WillInformation, path: string): 'document' | 'video' | 'audio' => {
    ...
    if (will.will_type === 'video' || will.will_type === 'audio') return will.will_type;
    return 'document';
  };
  ...
  const handleOpenExistingWill = async (will: WillInformation) => {
    ...
    const inferredWillType = inferWillType(will, path);
    const extension = getFileExtension(path);

    // Check if this is a digital will (generated by the app)
    const isDigitalWill =
      inferredWillType === 'document' &&
      (
        will.will_document_name?.includes('Digital Will') ||
        will.is_primary_will === true ||
        extension === 'html' ||
        extension === 'htm'
      );
    ...
    if (inferredWillType === 'document') {
      setDocumentLoading(true);

      try {
        const supportedTypes = ['pdf', 'html', 'htm'];
